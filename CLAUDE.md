...
Fenced code blocks render through `layouts/_default/_markup/render-codeblock.html` — a hook whose only job is wrapping Hugo's normal Chroma output with copy-to-clipboard (`static/js/modules/copy-button.js`) and expand-to-modal (`static/js/modules/code-expand.js`, one shared modal per page via `layouts/partials/code-modal.html`) buttons; it does not alter highlighting itself. The code theme (Chroma `vs`/`vulcan`) tracks the site's own `data-theme` the same way design tokens do. `.post-layout > article` needs `min-width: 0` — without it, an unbreakable long code line blows out the CSS grid track and the ToC sidebar disappears instead of just narrowing. Details: [`specs/components/posts.md`](specs/components/posts.md#подсветка-кода-code-blocks).
