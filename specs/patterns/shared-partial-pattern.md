...
`cover-image.html` и `assert-image-field.html` (валидация/рендер `cover` в
`posts/single.html`, см. [`components/posts.md`](../components/posts.md)) в эту
таблицу **не входят** — сейчас у обложки поста только одно место вызова, значит
это не переиспользуемый-partial-паттерн в смысле этой таблицы. Они остаются
отдельными файлами по другой причине: разделение «проверить форму» / «безопасно
отрендерить» само по себе полезно (см. заметку про `errorf` не прерывает
выполнение в ADR-8), а не ради переиспользования между несколькими шаблонами.
Параметры у них не симметричны с остальной таблицей по той же причине
(единственное место вызова), но оба всё же берут `dict`, не значение напрямую:
`cover-image.html` — `dict "cover" .Params.cover "page" .`, потому что резолвинг
`cover.src` как bundle-relative page resource (посты — Hugo page bundles, см.
[`data-model/content-organization.md`](../data-model/content-organization.md))
требует саму `Page`, не только объект `cover`; `assert-image-field.html` — `dict`
(`value`, `message`), потому что `message` собирается вызывающим шаблоном под
конкретный пост (`printf` с `.File.Path`). Ни то, ни другое не ради
переиспользования между несколькими шаблонами — `dict` здесь чисто потому, что
одного параметра не хватает по существу задачи.
...
## Пример закрытого контракта: `archive-item.html`
Три места вызова (архив постов, страница тега, страница серии) — это уже
устоявшийся мини-API, а не совпадение. Параметры (`page`, `linkText`, `hideDate`)
задокументированы прямо в самом partial (комментарий вверху
`layouts/partials/archive-item.html`), а не только здесь — так контракт виден и
при чтении кода, без перехода в specs. Если понадобится четвёртый архивный список
(например, будущая пагинация или новая группировка) — переиспользуйте этот partial
с тем же набором параметров, а не добавляйте новый ad hoc параметр под частный
случай. Если новому списку действительно нужно что-то принципиально другое —
это сигнал, что он не `archive-item`-паттерн, а отдельный partial.
