...
Поэтому и параметры у них не симметричны с остальной таблицей: `cover-image.html`
берёт значение напрямую как `.` (см. «Вариант: partial принимает `.` напрямую»
ниже) — раз он не переиспользуется, `wrapperClass`/`class`/`loading` через `dict`
были бы generic-параметрами без второго вызывающего, который бы ими
воспользовался. `assert-image-field.html` — другое дело, ему нужен `dict`
(`value`, `message`), потому что `message` собирается вызывающим шаблоном под
конкретный пост (`printf` с `.File.Path`), а не потому что partial переиспользуют
