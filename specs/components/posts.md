...
- `layouts/_default/_markup/render-codeblock.html` — render-hook. **Единственная**
  его цель — обернуть штатный вывод `transform.Highlight .Inner .Type` (тот же
  HTML, что Hugo сгенерировал бы и без хука) в `.code-block-wrapper` с кнопками
  copy и expand. Хук не подменяет и не настраивает сам хайлайтинг.
- Обе кнопки лежат в общем `.code-block-actions` (флекс-контейнер,
  ...
  кнопок раскрывает всю группу разом), а ниже мобильного порога `767px` виден
  всегда — у touch-устройств нет надёжного hover, кнопки иначе были бы
  недостижимы. Обе кнопки — класс-комбо с `.btn-control`/`.btn-control-icon`
...
- **Copy** (`.copy-btn`) — свап SVG-иконок (clipboard → checkmark на ~2 сек), по
  ...
  `main.js`): читает `.innerText` уже отрендеренного `<code>` (Chroma-спаны
  игнорируются сами по себе, без data-атрибута с сырым текстом) и пишет через
  `navigator.clipboard.writeText`. Без JS кнопка просто ничего не делает — код
  всё равно виден и выделяем вручную. `aria-label` — i18n-ключ `copyCodeAria`
  (статичный, не меняется при клике — меняется только иконка).
...
- `overflow-x: auto` на `.chroma` держит длинные строки (Chroma не переносит
  ...
  **сам по себе недостаточен**. `.post-layout` — CSS grid (`3fr 1.2fr`), и без
  ...
  колонка статьи раздувалась far past свою `3fr`-долю и физически вытесняла
