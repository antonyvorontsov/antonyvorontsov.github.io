...
Побочный эффект этого сочетания: ссылка «на главную» (`a.nav-avatar-link` в
`header.html`, через `.Site.Home.RelPermalink`) физически указывает на файл
`.../index.html` — у Hugo нет способа резолвить относительную ссылку на корень
раздела без имени файла. Менять сам `href` нельзя (сломает `file://`), поэтому
адресную строку косметически подчищает `navigation.js` → `initCleanHomeURL`:
после загрузки страницы по `http(s)://` (не `file:`) он через
`history.replaceState` убирает хвостовой `index.html`, оставляя `.../`. См.
