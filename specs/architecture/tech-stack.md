...
- ❌ **Сторонние CSS/JS-библиотеки** (jQuery, lodash, highlight.js, lunr.js…) —
  **единственное текущее исключение: Giscus** (комментарии на постах), см. ADR-9
  в [`decisions/architecture-decisions.md`](../decisions/architecture-decisions.md#adr-giscus-exception)
...
`{{ if .Scratch.Get "isCommentable" }}` (Giscus — тот же набор страниц, что и
`isPostSingle` (только посты), и только когда сайт реально сконфигурирован и
`enableComments` не `false`; см. [`components/comments.md`](../components/comments.md)),
поэтому на остальных страницах/сборках `window.Blog.copyButton`/`codeExpand`/`giscus`
вообще не существуют. `main.js` вызывает их через `if (copyButton) copyButton.init();` —
