...
`enableRobotsTXT = true` (`hugo.toml`) + `layouts/robots.txt` — шаблон, не статичный
файл в `static/`: строке `Sitemap:` нужен абсолютный URL, а `static/robots.txt` не
может собрать его без хардкода `vorontsov.dev`, который разрешён только в
`static/CNAME` (см. «Обработка домена» выше). Шаблон резолвит его через
`{{ "sitemap.xml" | absURL }}` — та же локальная/CI-подстановка `baseURL`, что и
везде: `/sitemap.xml` локально, `https://vorontsov.dev/sitemap.xml` в проде.

`sitemap.xml` — штатный Hugo-вывод (`disableKinds` его не гасит), отдельный по
языкам (`/ru/sitemap.xml`, `/en/sitemap.xml`) плюс индекс в корне. Ничего вручную
не настроено и не должно быть — Hugo сам исключает из него черновики (см. ниже) и
сам возвращает абсолютные URL независимо от `relativeURLs` (в отличие от
`.Permalink` в собственных шаблонах — см.
[`components/posts.md`](../components/posts.md#seo-og-теги-canonical-json-ld)).

**Черновики (`draft: true`, см.
[`frontmatter-reference.md`](../data-model/frontmatter-reference.md#пост))
исключаются из сборки полностью**, не только из индексации: ни `hugo.yml`, ни
`hugo-pr-check.yml` не передают `-D`/`--buildDrafts`, значит страница с
`draft: true` не попадает в `public/` вообще — а раз её физически нет, её нет и в
`sitemap.xml`, RSS, `search-index.json`, архиве постов, `.Site.RegularPages` (в т.ч.
для страниц серий) и, разумеется, в robots.txt-исключениях — исключать их там
дополнительно не нужно. Локальный `hugo server`/`hugo` ведут себя так же (drafts
не строятся без явного `-D`), т.е. черновик не виден и в дев-превью — используйте
`hugo server -D`, если нужно посмотреть черновик локально.
